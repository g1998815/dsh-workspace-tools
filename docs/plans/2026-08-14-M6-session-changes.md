...
{ callId, tool: "write"|"edit", file: displayPath, abs: processPath,
  ...
  sessionId, turn: number /* 对话轮次（0 起） */, step: number, at: epochMs }

// store：
//   pending:   Map<sessionId, Array<Record>> —— 未处理（按 turn, step 升序）
//   processed: Map<sessionId, Array<{rec, action: "adopted"|"reverted", handledAt}>> —— 已处理历史
```

> **用户裁定（2026-08-16，三轮）**：
> 1. 已采用/已撤回记录不在待处理列表展示——`adopt`/`revert` 后从 pending 移出。
> 2. 新增**已处理历史**：操作过的记录进入历史区（tab 下方），**按对话分组**，**每会话最多保留 10 个对话（turn）**（超出丢最旧对话的全部记录）；提供**清除**按钮清空该会话历史。
> 3. **展示按"对话 → 文件"分组**：一个对话（用户发一条消息 = 一个 turn）可能改多个文件——待处理区与历史区都按对话分组，对话标题（"对话 #N" + 时间/条数）下挂该对话修改的文件列表；"10 个对话"按 turn 计数，不是按文件计数。
> 4. turn 归属：监听 `session/event`（`tool/call` 事件 data 含 `{turn, step, callId, name}`）建立 callId→turn 映射；`tools/result` 按 callId 取 turn。撤回动作本身是 host 直接写文件（不走工具），不会形成新记录循环。

## RPC 端点（`lib/index.js` 追加，沿用 `(endpoint,payload)` 信封 + guardCwd）

| 端点 | payload | 返回 |
|---|---|---|
| `sessionChanges.list` | `{sessionId, cwd}` | `{items: Record[]}`（当前会话**全部 pending** 记录，含 turn/step） |
| `sessionChanges.history` | `{sessionId, cwd}` | `{items: Array<{rec, action, handledAt}>}`（当前会话已处理历史，按对话分组 ≤10 对话） |
| `sessionChanges.clearHistory` | `{sessionId, cwd}` | `{ok:true}`；清空该会话历史 |
| `sessionChanges.adopt` | `{sessionId, cwd, callId}` | `{ok:true}`；pending → 移入 processed（action="adopted"，按对话截断 ≤10） |
| `sessionChanges.revert` | `{sessionId, cwd, callId}` | `{ok:true}`；写回 before（新增文件→`fs.rm`）→ 移入 processed（action="reverted"）；失败结构化错误 |
...
**Interfaces:**
- Produces:
  - `createSessionChangesStore()` → `{ list(sessionId), push(sessionId, rec), find(sessionId, callId), adopt(sessionId, callId), history(sessionId), clearHistory(sessionId) }`
  - `captureIntents(ctx, store)`：注册 write/edit-intent 监听（读 before，记 pending by callId）
  - `captureResults(ctx, store)`：注册 tools/result 监听（匹配 pending → 组装 Record → push；turn 从 callId→turn 映射取）
  - `captureTurnMap(ctx, store)`：注册 `session/event` 监听（`tool/call` 事件 data `{turn, step, callId, name}` → 记录 callId→{turn, step}）
  - `revertRecord(store, sessionId, callId, { fs, path })`：写回/删除；成功后移入 processed（action="reverted"）
- Consumes: `ctx.waterfall` / `ctx.on` / `ctx.fs`（readText/processPath）、`node:fs/promises`（rm）。

- [ ] **Step 1: 写失败测试（纯逻辑）**

```js
// test/session-changes.test.js —— 记录模型/组装/分组/截断（无 Node 依赖的纯函数优先）
// 用例：push+list 顺序；同 turn 多文件 → 多条；step/turn 自增；
//      adopt/revert → pending 移出、processed 追加（action 正确）；
//      processed 按 turn 分组、**对话数上限 10**（超出丢最旧对话全部记录）；
//      clearHistory 清空；幂等（重复操作不存在记录返回 ok）；
...
- Produces:
  - `SessionChanges({ cwd, sessionId, rpc, onCountChange })`：
    - 加载 `sessionChanges.list` → 当前会话 pending 记录 → **按对话（turn）分组**
    - 对话组标题："对话 #N" + 该对话修改文件数；组内列出每条（文件名 + 时间 + 工具，展开看 before/after diff）
    - 每条按钮：**采用**、**撤回**（操作成功 → 该条移出 pending + `onCountChange` 更新计数）
    - `onCountChange(n)`：pending 记录总数（非对话数）→ tab 标题显示"会话变更 N"
    - **已处理历史区（tab 下方）**：加载 `sessionChanges.history` → **按对话分组**（≤10 个对话，倒序）；对话标题"对话 #N"，组内每条显示 action 徽标（已采用/已撤回）+ 文件名 + 时间；**清除**按钮 → `clearHistory` 后清空本地列表
...
```js
// 加载 → 按 turn 分组 → 对话组（标题 + 文件列表）→ 展开/收起 → 操作按钮（采用/撤回）
// → 下方已处理历史区（按对话分组 ≤10 + 清除按钮）
// 标记：data-wt-sesschg（根）、data-wt-sesschg-turn（对话组）、data-wt-sesschg-item、
//       data-wt-sesschg-adopt、data-wt-sesschg-revert、data-wt-sesschg-diff、
//       data-wt-sesschg-history、data-wt-sesschg-clear
```
...
- [ ] 待处理区按**对话分组**：对话 #N 标题 + 该对话修改的文件列表；展开可见 before/after
- [ ] 采用 → 该条移出 pending + 计数减一，并出现在下方"已处理历史"对应对话下（已采用）
- [ ] 撤回 → 文件恢复 + 该条移出 pending + 计数减一，历史区对应对话出现"已撤回"
- [ ] 历史区按对话分组、最多 10 个对话（超出丢最旧对话全部记录）；"清除"按钮清空
...
- [ ] write/edit 工具修改文件 → 会话变更记录生成（before/after/对话归属）。
- [ ] 采用 / 撤回可用；撤回恢复 before（新增文件删除）；**操作后记录移入已处理历史（按对话分组，每会话最多 10 个对话，超出丢最旧）**；幂等。
- [ ] tab 标题显示未处理计数（"会话变更 N"，按记录数），随操作实时更新。
- [ ] 已处理历史区按对话分组展示（已采用/已撤回徽标 + 文件名 + 时间）；"清除"按钮清空。
- [ ] 与 git 解耦：不产生 git 提交；撤回作为新修改反映到"变更"tab。
- [ ] GUI："会话变更"tab 展示（对话 → 文件）、展开 diff、操作反馈、空态。
- [ ] 推送 GitHub main。
