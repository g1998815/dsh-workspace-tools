...
// 注意：waterfall 监听器用 ctx.on 注册（ctx.waterfall 是触发 API，不是注册）；
// 且必须 prepend（先于 fs-observation-policy 执行）——policy 监听器短路返回
// 决策（不调 next），若在其后注册，本监听器永远不会执行，before 无法捕获。
export function captureIntents(ctx, pendingIntents = new Map()) {
  if (typeof ctx.on !== "function") return pendingIntents;
  ...
  ctx.on("fs/write-intent", handler, { prepend: true });
  ctx.on("fs/edit-intent", handler, { prepend: true });
...
export function captureResults(ctx, store, pendingIntents = new Map(), turnMap = new Map()) {
  ...
  ctx.on("tools/result", async (exec) => {
    ...
    if (!intent) {
      turnMap.delete(callId); // 非 write/edit 工具也清理 turnMap，防泄漏
      return; // 非 write/edit 工具或意图未捕获
    }
